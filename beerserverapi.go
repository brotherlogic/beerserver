...
func (s *Server) Update(ctx context.Context, req *pb.UpdateRequest) (*pb.UpdateResponse, error) {
	...
	if time.Now().Hour() < 14 && (time.Now().Weekday() == time.Monday || time.Now().Weekday() == time.Wednesday || time.Now().Weekday() == time.Friday || time.Now().Weekday() == time.Tuesday || time.Now().Weekday() == time.Thursday) {
